...
It shouldn't come as a surprise that logistic regression was included in our survey algorithms. 

In particular, we will be investigating the \textbf{glmnet} package as it is intended for large datasets. \texttt{glmnet} fits a generalized linear model via penalized maximum likelihood (4). It boasts a robust implementation that can handle very large sparse matrices (6), a particular data structure in \texttt{R}. As such, using \textbf{glmnet} would enable training on data even a hundred times larger than what the other methods studied here can handle. Of course, training on such large data will still be very slow and memory-intensive even with this particular package.
